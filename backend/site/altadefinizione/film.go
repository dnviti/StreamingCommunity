...
func GetFilmDetails(filmURL string) (*FilmDetails, error) {
	...
	// For now, we'll just look for a common video player embed or link
	videoURL, exists := doc.Find("iframe").Attr("src")
	if exists {
		film.VideoURL = videoURL
	}

	// Attempt to find HLS video URL (this is a very basic example and might need refinement)
