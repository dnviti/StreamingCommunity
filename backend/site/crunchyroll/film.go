...
func GetFilmDetails(filmURL string) (*FilmDetails, error) {
	...
	// Extract Title
	film.Title = doc.Find("h1.hero-title").Text()

	// Extract Description
	film.Description = doc.Find(".description-text").First().Text()

	// Placeholder for VideoURL - this will require more advanced scraping
	// Crunchyroll uses a complex player, so this will likely involve yt-dlp or similar
